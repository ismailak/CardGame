...
namespace CardGame.Wheel
{
    ...
    {
        [Header("Design")] [SerializeField] private WheelData[] _wheelDatas;
        ...
        [Space, Header("Development")] [SerializeField]
        ...
        public void SetNewWheel()
        {
            ...
            else
            {
                ...
                else if (GameManager.Instance.CurrentWheelLevel % _wheelSettings.GoldInterval == 0)
                    targetWheelData = _spareGoldWheelDatas[Random.Range(0, _spareGoldWheelDatas.Length)];
                else if (GameManager.Instance.CurrentWheelLevel % _wheelSettings.SilverInterval == 0)
            ...
            _wheelPanelTransform.DOAnchorPosY(_wheelSettings.YPosition, _wheelSettings.WheelReloadDuration / 2f)
                .OnComplete(() => { SpinManager.Instance.ChangeInteractable(true); });
        }
